...
(** ** Test for a warning *)
Goal True.
ltac2: assert_feedback_with_string (fun () => warn (Message.of_string "Send a first warning.")) Warning "Send a first warning.".
Abort.

(** ** Test if a warning is really thrown (and not just the last item in the log is compared) *)
Goal True.
ltac2: warn (Message.of_string "Send a warning.").
ltac2: assert_fails_with_string (fun () => assert_feedback_with_string (fun () => ()) Warning "Send a warning.")
"The tactic was expected to warn 1 times, but it warned 0 times.".
Abort.

(** ** Test asserting for a failure with a given string *)
Goal True.
ltac2: assert_fails_with_string (fun () => throw (Message.of_string "This error _should_ be raised.")) "This error _should_ be raised.".
Abort.
...
Goal True.
ltac2: assert_feedback_with_string (fun () => warn (Message.of_string "warning
