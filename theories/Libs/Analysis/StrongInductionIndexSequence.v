...
Proof.
  ltac2: enough (sig (fun (seq : nat -> A) => forall k : nat, forall l : nat, l <= k -> P l seq)) as [seq Hseq].
  { ltac2: exists seq.
...
Proof.
  ltac2: enough (sig (fun (n : nat -> nat) => forall k : nat, index_seq_prop_family Q k n)) as [n Hn].
  { ltac2: exists n.
...
Proof.
  ltac2: destruct H0 as [n0 Hn0].
  ltac2: assert (sig (fun n0 => Q n0)) as H0sig by (exists n0; exact Hn0).
  ...
  ltac2: enough (sig (fun (n : nat -> nat) => is_index_seq n /\ (forall k : nat, Q (n k)))) as [n Hn].
...
Waterproof Notation "Define" "the" "index" "sequence" "n" "inductively" :=
...
Waterproof Notation "We" "first" "define" "n_0" :=
  strong_ind_indx_base ().

Waterproof Notation "Take" "k" "∈" "ℕ" "and" "assume" "n_0,...,n_k" "are" "defined" :=
  take_first_k ().
