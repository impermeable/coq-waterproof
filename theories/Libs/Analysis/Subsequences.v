...
(* TODO: this file is not fully converted to the new notations, but the definitions are *)
...
Lemma existence_next_el_to_fun (a : ℕ → ℝ) (P : ℕ → ℝ → Prop) :
...
Lemma created_seq_is_index_seq (g : ℕ → ℕ → ℕ) :
    (∀ m : ℕ, ∀ N : ℕ, (g m N ≥ N)%nat ) ⇒
...
Lemma subseq_sat_rel (a : ℕ → ℝ) (g : ℕ → ℕ → ℕ) (P : ℕ → ℝ → Prop) :
    (∀ m : ℕ, ∀ N : ℕ, P m (a (g m N)) ) ⇒
      ∀ k : ℕ, P k (a (create_seq g k)).
Proof.
    Assume that (∀ m : ℕ, ∀ N : ℕ, P m (a (g m N))) as (i).
    ltac2: induction k. (*TODO: if we use 'We use induction on k.'-tactic, then we cannot directly match on k + 1 *)
...
Lemma exists_good_subseq (a : ℕ → ℝ) (P : ℕ → ℝ → Prop) :
    (∀ (m : ℕ), ∀ (N₀ : ℕ), ∃ k : ℕ, (N₀ ≤ k)%nat ∧ (P m (a k))) ⇒
      ∃ n : ℕ → ℕ, is_index_seq n ∧ ∀ k : ℕ, P k (a (n k)).
Proof.
    Assume that ∀ m : ℕ, ∀ N : ℕ, there exists k : ℕ , (N ≤ k)%nat ∧ P m (a k).
...
Proof.
    We need to show that ((∀ k ∈ ℕ, (g k ≤ g (S k))%nat)
      ⇨ ∀ k : ℕ, ∀ l : ℕ, (k ≤ l)%nat ⇨ (g k ≤ g l)%nat).
...
Lemma index_seq_strictly_incr (n : ℕ → ℕ) :
  is_index_seq n ⇒ (is_increasing (fun (k : ℕ) ↦ (n k - k)%nat)).
Proof.
    Assume that is_index_seq n.
    We need to show that ∀ k ∈ ℕ, (n(k) - k ≤ n(S(k)) - S(k))%nat.
    Take k ∈ ℕ.
...
Lemma index_seq_grows (n : ℕ → ℕ) :
  is_index_seq n ⇒ (∀ k : ℕ, ∀ l : ℕ, (k ≤ l)%nat ⇒ (n k - k ≤ n l - l)%nat).
Proof.
    Assume that is_index_seq n.
...
Lemma inf_el_to_fun (a : ℕ → ℝ) (P : ℕ → ℝ → Prop) :
...
Lemma seq_of_max_is_increasing (g : ℕ → ℕ) :
  is_increasing (seq_of_max g).
Proof.
...
Lemma elements_le_seq_of_max (g : ℕ → ℕ) (n : ℕ) (k : ℕ) :
...
Lemma built_seq_is_index_seq (g : ℕ → ℕ) :
    (∀ k : ℕ, (g k ≥ k)%nat) ⇒
      is_index_seq (build_seq g).
Proof.
    Assume that for all k : ℕ, (g k ≥ k)%nat.
    We need to show that ∀ k ∈ ℕ, (build_seq g k < build_seq g (S k))%nat.
    Take k ∈ ℕ.
